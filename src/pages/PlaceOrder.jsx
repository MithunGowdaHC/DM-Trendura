        ...
        <div className=" mt-12">
          ...
          <div className=" flex gap-3 sm:flex-col lg:flex-row ">
            ...
            <div onClick={()=>setMethod('stripe')} className=" flex items-center  gap-3   border p-2 px-3  cursor-pointer">
              ...
              <img src={assets.stripe_logo} className=" h-5 w-full sm:w-[6rem] " alt="" />

            </div>
            <div onClick={()=>setMethod('razorpay')} className=" flex items-center gap-3 border p-2 px-3  cursor-pointer">
              <p className={` min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? ' bg-green-400' : ''} `}></p>
              <img src={assets.razorpay_logo} className=" h-5 w-full sm:w-[7rem] " alt="" />

            </div>
            <div onClick={()=>setMethod('cod')} className=" flex items-center gap-1 border p-2 px-3  cursor-pointer">
              <p className={` min-w-3.5 h-3.5 border  rounded-full ${method === 'cod' ? ' bg-green-400' : ''}`}></p>
            <p className=" text-gray-500  w-full  text-sm font-medium mx-4">CASH ON DELIVERY</p>
            </div>
